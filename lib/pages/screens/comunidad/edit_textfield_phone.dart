...
class EditTextFieldPhoneCommunity extends StatefulWidget {
  final Map<String,dynamic>? phone;
   const EditTextFieldPhoneCommunity({super.key,this.phone});

  @override
  State<EditTextFieldPhoneCommunity> createState() => _InputEditState();
}

class _InputEditState extends State<EditTextFieldPhoneCommunity> {
  ...
  void initState() {
    ...
    super.initState();
      _textEditingController.text = widget.phone!["number_phone"];
                ...
                onPressed: () async {
                  if (usernameStatus) {
                    ...
                    final response = await ApiServices().updateTextField(
                      {
                        "number_phone" : _textEditingController.text
                      }, 
                      widget.phone!["id"]);
                    ...
                    if (response == StatusAccount.successfully) {
                       await SaveUserSharedPreferences().saveUserComunnity({
                        ...
                        "lastname": widget.phone!["lastname"],
                        "mail": widget.phone!["mail"],
                        "photo_profile": widget.phone!["photo_profile"],
                        "number_phone": _textEditingController.text,
                    ...
                    if (response == StatusAccount.errorTimeOut) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Tiempo de espera acabo intentelo de nuevo")));
                    ...
                    if (response == StatusAccount.badGateway) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Fuera de servicio")));
